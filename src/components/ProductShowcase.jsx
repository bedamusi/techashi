    ...
    <section id="products" className="py-24 sm:py-32 bg-white relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {!loading && filteredProducts.length > 0 && (
          <div className="mb-16">
            <div className="mb-5 flex items-end justify-between"><h3 className="text-2xl font-heading font-bold text-brand-navy">Products</h3><span className="text-[11px] text-slate-400">{filteredProducts.length} items</span></div>
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {filteredProducts.map((product) => (
                <article key={product.id} className="overflow-hidden border-y border-slate-200 bg-white transition-colors hover:bg-slate-50">
                  ...
                  <div className="p-5">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-brand-blue">{PRODUCT_CATEGORIES.find((category) => category.id === product.category)?.name}</p>
              ...
              >
                ...
                <div className="p-8 pb-4">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-white border border-slate-200 text-brand-blue shadow-2xs">
                      {cat.badge}
                    </span>
                ...
                <div className="p-8 pt-4 border-t border-slate-100 bg-white">
                  ...
                  <button
                    onClick={onOpenQuote}
                    className="w-full py-3 rounded-xl bg-slate-50 hover:bg-brand-blue hover:text-white border border-slate-200 hover:border-transparent text-slate-800 text-xs font-heading font-semibold transition-all flex items-center justify-center gap-1.5 group-hover:bg-brand-blue group-hover:text-white"
                  >
