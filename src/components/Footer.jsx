...
function SocialIcon({ mark }) {
  ...
  if (mark === 'instagram') {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true" className="h-[19px] w-[19px] fill-none stroke-current" strokeWidth="1.8">
  ...
  return (
    <footer className="bg-slate-900 text-slate-300 pt-16 pb-12 border-t border-slate-800 text-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Apple-Style Footnotes & Disclaimers (ac-gf-sosumi) */}
        <div className="pb-10 mb-12 border-b border-slate-800 text-[11px] text-slate-400 space-y-2 leading-relaxed">
          <p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-10 pb-16 border-b border-slate-800">
          ...
          <div className="col-span-2 md:col-span-2 space-y-4">
            ...
            <div className="pt-2">
              <button
                onClick={onOpenQuote}
                className="px-4 py-2 rounded-full bg-brand-blue text-white font-heading font-semibold text-xs hover:bg-brand-blue-hover transition-colors"
              >
              ...
              <div className="flex items-center gap-2 pt-1">
                {SOCIAL_PLATFORMS.map(({ name, mark }) => (
                  <span
                    ...
                    aria-label={name}
                    title={name}
                    className={`flex h-9 w-9 items-center justify-center rounded-full border border-slate-700 bg-slate-800/70 transition-colors hover:border-slate-500 hover:bg-slate-700 ${
        ...
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-slate-500">
          ...
          <div className="flex items-center gap-6">
            <span>Official Domain: <a href="https://techashisolutions.com/" className="text-slate-400 hover:text-white transition-colors">techashisolutions.com</a></span>
          </div>
        </div>
